...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	log.Info(`BootstrapNodes`)
	urls := params.MainnetBootnodes
	////fmt.Println("读取默认区块服务器链接 quyum",urls)
	switch {
...
// flags, reverting to pre-configured ones if none have been specified.
//setBootstrapNodesV5从命令行标志创建一个引导节点列表，如果没有指定，则恢复为预先配置的节点

func setBootstrapNodesV5(ctx *cli.Context, cfg *p2p.Config) {

	urls := params.DiscoveryV5Bootnodes
	log.Info(`DiscoveryV5Bootnodes`)
	log.Info(urls[0])
	switch {
	...
	////fmt.Println(cfg.BootstrapNodesV5)
}
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "BHB", ethPeers, "LES", lightPeers, "total", cfg.MaxPeers)
	...
	if ctx.GlobalBool(DeveloperFlag.Name) {
		...
		////fmt.Println("这里执行的是dev模式")
		cfg.MaxPeers = 0
		cfg.ListenAddr = ":0"
		cfg.NoDiscovery = true
		cfg.DiscoveryV5 = false
	}
	////fmt.Println(cfg.DiscoveryV5)
}
...
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the CCM service: %v", err)
	}
}
